...
// Initialize the menu navigation action
			...
			setTimeout( function(){				
				...
				if(self.url !== self.homePg || !self.onePage){
					...
					if(self.onePage){
						setTimeout( function(){	 
							try{ 
								self.scrollTopAction(self.htmlScroll, "0px", 20);
								} catch (e) { }	 
						}, 500);
					}
				}
		...
		vertical_scroll : function(){
			...
			if(!isTouch){ scrIntervalTim = scrMenuIntervalTim = 30; }
			...
			try{  
				...
				if(self.chkHideMc !== self.curScrlPgMc.data("data-id")){							
					...
					try{
						...
						if(self.owlSliderArry.length > 0 && !isTouch){									
							for(var hir=0; hir < self.owlSliderArry.length; hir++){
								if(self.owlSliderArry[hir].data("owlLoad")){										
									self.owlSliderArry[hir].data('owlCarousel').stop();	
								}
							}
						}					
						
						if(self.curScrlPgMc.data("owlLoad") && !isTouch){									
							for(var owlAry=0; owlAry < self.curScrlPgMc.data("owlArryZ").length; owlAry++){										
								if( self.curScrlPgMc.data("owlArryZ")[owlAry].data("owlLoad") && self.curScrlPgMc.data("owlArryZ")[owlAry].data("playIt")){											
									self.curScrlPgMc.data("owlArryZ")[owlAry].data('owlCarousel').play();
								}
							}
						}
		...
		beforePageLoad : function(ele){
			...
			for(var ab=0; ab < self.conArry.length; ab++){
				if(self.conArry[ab].attr("data-id") === self.url){
					...
					try{ 
						if(self.conArry[ab].data("owlLoad")){
							self.conArry[ab].find('.owlSlider').each(function(){							
